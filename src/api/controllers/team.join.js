 ...
 */
module.exports = (app) => {
  app.post('/team/:id/join', [
    isAuth('team-join'),
    isNotInTeam('team-join')
  ])
  ...
  app.post('/team/:id/join', async (req, res) => {
    if (req.user.teamId !== null) {
      ...
      return res.status(400).json({ error: 'ALREADY_IN_TEAM' }).end()
    }

    try {
      const team = await req.app.locals.models.Team.findById(req.params.id, { include: [ User, Spotlight ] })
